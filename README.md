...
### Store purchases while the app is in closed testing

Two settings decide whether a purchase the store marks `SANDBOX` grants a plan,
...
```bash
# .env on the host
BILLING_ACCEPT_SANDBOX=false
```

`env.ts` computes `acceptSandbox` as `BILLING_ACCEPT_SANDBOX === 'true' ||
NODE_ENV !== 'production'`, so the flag is only half the answer. With
`NODE_ENV` unset the fallback clause is true and prod honours sandbox
purchases whatever the flag says — the state this deployment was in until
2026-08-25, when every billing event on it was a free test purchase. Both
lines are needed; setting one without the other does nothing.

A tester whose Google account is on **Play Console → Setup → License testing**
sees "Test card, always approves" and "You will not be charged" on the purchase
sheet. That is an account-level list, not a property of the track — a closed
tester who is not on it pays real money, and a license tester sees test cards on
every track including production.

So the expected outcome for a license tester on a correctly configured
deployment is: the sheet accepts the test card, `POST /billing/revenuecat`
answers `{"applied":false,"reason":"wrong_environment"}`, no row is written to
`billing_events`, and the app polls for ten seconds (`awaitPlan` in
`apps/mobile/lib/billing.ts`) before reporting that the purchase failed. **A
tester saying "it took my test card and nothing happened" is this working, not a
bug.** Two ways to change it, depending on what you are testing:

- **To have testers exercise a real purchase** — take them off the license
  testing list. Propagation takes minutes and Play caches it, so have them
  force-stop the Play Store app before retrying.
- **To re-verify the webhook loop yourself** — set
  `BILLING_ACCEPT_SANDBOX=true` on the host, recreate the api container, buy
  on a license-tester account, then put it back. Never leave it true: a
  sandbox purchase costs nothing, so honouring one is a free Coach plan for
  anybody on that list.

Verify the guard without a phone — this writes nothing and grants nothing:

```bash
curl -s -X POST https://api.daysofar.com/billing/revenuecat \
  -H "Authorization: $REVENUECAT_WEBHOOK_SECRET" \
  -H "Content-Type: application/json" \
  -d '{"event":{"id":"guard-check","type":"INITIAL_PURCHASE",
       "app_user_id":"not-a-real-user","product_id":"coach:annual",
       "environment":"SANDBOX"}}'
# {"ok":true,"applied":false,"reason":"wrong_environment"}
```

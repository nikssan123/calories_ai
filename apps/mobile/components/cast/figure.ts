 ...
 * data. `Character` renders it as react-native-svg layers it can move. The
 * Android widget renders the same data as an SVG string for a launcher to
 * rasterise. One drawing, two renderers, like the ring's arithmetic, which
 * `ring.ts` shares with `CalorieRing`. The figure on the home screen is the
 * figure in the app, not a second drawing of the same idea.
 *

...
/**
 * The brand's own accounts, in one list because two places have to agree.
 *
 * `sameAs` in the Organization node says these profiles are this entity, and
 * the footer shows them — `lib/schema.ts` opens with the rule that structured
 * data may only claim what the page also shows, and a `sameAs` nobody can click
 * is exactly the half-claim that rule is about.
 *
